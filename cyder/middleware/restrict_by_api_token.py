...
class RestrictByToken:
    ...
    def authenticate(self, username=None, password=None, token=None):
        ...
        token = None
        try:
            the_part = base64.b64decode(
                request.META['HTTP_AUTHORIZATION'].split()[1]).split(':', 1)
            username = the_part[0]
            password = the_part[1]
        except:
...
class AuthenticatedAPI(object):
    ...
    def __call__(self, request, *args, **kwargs):
        ...
        if self._using_token is True:
            try:
                the_part = base64.b64decode(request.META['HTTP_AUTHORIZATION']
                                            .split()[1]).split(':', 1)
            except Exception, e:
                print e
            try:
                part1 = base64.b64decode(
                    request.META['HTTP_AUTHORIZATION'].split()[1])
                username = part1.split(":")[0]
                password = part1.split(":")[1]
                user = User.objects.get(username__exact=username)
                user_profile = property(
                    lambda u: UserProfile.objects.get_or_create(user=user)[0])
