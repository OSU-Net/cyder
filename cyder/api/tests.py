...
class SystemApi(TestCase):
    ...
    def test_get_system_by_hostname(self):
        resp = self.client.get(
            '/api/system/asfdasfasfasdfasfasdfsadf/', follow=True)
        self.assertEqual(404, resp.status_code)
    ...
    def test_create_system(self):
        resp = self.client.post(
            '/en-US/api/system/%s/' % self.new_hostname, follow=True)
        self.assertEqual(201, resp.status_code)
        resp = self.client.get('/api/system/%i/' % 3, follow=True)
        self.assertEqual(200, resp.status_code)
        resp = self.client.get(
            '/api/system/%s/' % self.new_hostname, follow=True)
        self.assertEqual(200, resp.status_code)
        self.assertEqual(
    ...
    def test_update_system(self):
        ...
        resp = self.client.put('/en-US/api/system/%i/' % (
    ...
    def test_delete_system(self):
        ...
        resp = self.client.delete(
            '/en-US/api/system/%i/' % (obj['id']), follow=True)
        self.assertEqual(200, resp.status_code)
        obj = json.loads(resp.content.split(" = ")[1])
        resp = self.client.get('/api/system/%i/' % (obj['id']), follow=True)
        self.assertEqual(404, resp.status_code)
    ...
    def test_key_value_api(self):
        resp = self.client.get(
            '/api/keyvalue/?keystore=fake-hostname2', follow=True)
        #print resp.content
        ...
        self.assertEqual(json.loads(
            resp.content)['host:fake-hostname1:ip_address'], '10.99.32.3')
...
class DHCPApi(TestCase):
    ...
    def test_get_single_scope(self):
        resp = self.client.get(
            '/api/keyvalue/?key_type=dhcp_scopes', follow=True)
