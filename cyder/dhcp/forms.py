...
class AddDHCPScopeForm(forms.Form):
    scope_name = forms.CharField(max_length=32, required=True,
...
class DHCPScopeOverrideForm(forms.ModelForm):
    dhcp_scope = forms.CharField(max_length=32, required=True,
                                 widget=forms.TextInput(attrs={'size': '48'}))
...
class EditDHCPScopeForm(forms.Form):
    ...
    allow_booting = forms.CharField(
        max_length=64, required=False, widget=forms.Select(choices=CHOICES))
    allow_bootp = forms.CharField(
        max_length=64, required=False, widget=forms.Select(choices=CHOICES))
