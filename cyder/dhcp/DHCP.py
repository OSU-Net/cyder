class DHCP:
    ...
    def header(self):
        ...
        self.header_text = 'subnet ' + self.scope_options['network_block'] + \
            ' netmask ' + self.scope_options['subnet_mask'] + ' {\n'
    ...
    def options(self):
        if 'option_ntp_servers' in self.scope_options:
            self.options_text = 'option ntp-servers ' + \
                self.scope_options['option_ntp_servers'] + ";\n"
        if 'option_subnet_mask' in self.scope_options:
            self.options_text += 'option subnet-mask ' + \
                self.scope_options['option_subnet_mask'] + ";\n"
        if 'option_domain_name' in self.scope_options:
            self.options_text += 'option domain-name "' + \
                self.scope_options['option_domain_name'] + "\";\n"
        if 'option_domain_name_servers' in self.scope_options:
            self.options_text += 'option domain-name-servers ' + \
                self.scope_options['option_domain_name_servers'] + ";\n"
        if 'option_routers' in self.scope_options:
            self.options_text += 'option routers ' + \
                self.scope_options['option_routers'] + ";\n"
        if 'option_subnet_mask' in self.scope_options:
            self.options_text += 'option subnet-mask ' + \
                self.scope_options['option_subnet_mask'] + ";\n"
        if 'filename' in self.scope_options:
            self.options_text += 'filename "' + \
                self.scope_options['filename'] + "\";\n"
