...
class TruthTest(TestCase):
    ...
    def test_get_dhcp_scope_by_name(self):
        resp = self.client.get(
            '/api/keyvalue/?keystore=phx-vlan73', follow=True)
    ...
    def test_get_dhcp_key_value(self):
        ...
        resp = self.client.get(
            '/api/keyvalue/?keystore=%s' % dhcp_scope, follow=True)
        obj = json.loads(resp.content)
        #self.assertEqual(2,len(obj))
        #self.assertEqual(obj['truth:phx-vlan73:is_dhcp_scope'], 'True')
    ...
    def test_dhcp_scope_names_and_descriptions(self):
        dhcp_scope = 'phx-vlan73'
        resp = self.client.get('/api/v2/dhcp/%s/get_scopes_with_names/' %
    ...
    def test_update_dhcp_scope_value(self):
        dhcp_scope = 'phx-vlan73'
        resp = self.client.put('/en-US/api/keyvalue/%s/' % dhcp_scope, {
                               'key': 'dhcp.scope.start', 'value': '1.1.1.1'})
        resp = self.client.put('/en-US/api/keyvalue/%s/' % dhcp_scope, {'key':
                               'dhcp.scope.start2', 'value': '9.9.9.9'})
