...
class Rack:
    ...
    def __init__(self, rack_name):
        ...
        self.kv = Truth.objects.select_related(
            'truth_key_value').get(name=self.rack_name)
        self.system_list = KeyValue.objects.select_related(
            'system').filter(value__contains="truth:%s" % (self.rack_name))
        self.ethernet_patch_panel_24 = self._get_ethernet_patch_panels(
            self.kv, 'ethernet', 24)
        self.ethernet_patch_panel_48 = self._get_ethernet_patch_panels(
            self.kv, 'ethernet', 48)
        for s in self.system_list:
            #print dir(s)
            client = Client()
            resp = client.get('/api/v2/keyvalue/?keystore=%s' %
                              (s.system.hostname), follow=True)
