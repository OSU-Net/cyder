...
def get_ldap_dn(email):
    try:
        ...
        res = conn.search_s(
            'dc=mozilla', ldap.SCOPE_SUBTREE, '(mail=%s)' % email, ['dn'])
        return res[0][0]
    except Exception, e:
        return None
...
def get_ldap_groups(dn):
    try:
        conn = ldap.initialize('ldap://%s/' % settings.LDAP_HOST)
        conn.simple_bind_s(settings.LDAP_USER, settings.LDAP_PASS)
        res = conn.search_s('ou=groups,dc=mozilla',
                            ldap.SCOPE_SUBTREE, '(member=%s)' % dn, ['cn'])
...
def get_all_names():
    ...
    conn.simple_bind_s(settings.LDAP_USER, settings.LDAP_PASS)
    res = conn.search_s(
        settings.LDAP_SEARCH_PATH, ldap.SCOPE_SUBTREE, '(mail=*)', ['cn'])
