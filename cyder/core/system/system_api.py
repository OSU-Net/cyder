...
import cyder.core.system.models as system_model
...
from cyder.cydhcp.lib.utils import create_ipv4_intr_from_domain
import json
import re
...
class SystemResource(CustomAPIResource):
    ...
    key_value = fields.ToManyField('systems.system_api.KeyValueResource',
                                   'keyvalue_set', full=True, null=True)
    server_model = fields.ForeignKey('systems.system_api.ServerModelResource',
                                     'server_model', null=True, full=True)
    system_status = fields.ForeignKey('systems.system_api.SystemStatusResource',
                                      'system_status', null=True, full=True)
    operating_system = fields.ForeignKey('systems.system_api.OperatingSystemResource', 'operating_system', null=True, full=True)
    system_rack = fields.ForeignKey('systems.system_api.SystemRackResource',
                                    'system_rack', null=True, full=True)
    allocation = fields.ForeignKey('systems.system_api.AllocationResource',
                                   'allocation', null=True, full=True)
    """
        ...
        interface = fields.ToManyField('systems.system_api.StaticInterfaceResource', 'staticinterface_set', null=True, full=True)
    """
    ...
    def extract_nic_attrs(nic_name):
        ...
        if m:
            ...
            raise ValidationError(
                "Invalid format for adapter name. ex: eth0.0")
    ...
    def obj_create(self, bundle, request, **kwargs):
        ret_bundle = super(
            SystemResource, self).obj_create(bundle, request, **kwargs)
    ...
    def obj_update(self, bundle, request, **kwargs):
        for intr in bundle.obj.staticinterface_set.all():
            intr.update_attrs()
        ret_bundle = super(
            SystemResource, self).obj_update(bundle, request, **kwargs)
...
class OperatingSystemData(CustomAPIResource):
    ...
    def get_data(self, data):
        data.set('id', '8')
        data.set('name', 'RHEL')
        data.set('resource_uri', '/tasty/systems/operating_system/8/')
