...
class MacroExpansion:
    ...
    def host_macro(self):
        try:
            ...
            self.output_text = KeyValue.objects.filter(
                system=system).filter(key=key)[0].value
            self.output()
        except Exception, e:
    ...
    def truth_macro(self):
        try:
            ...
            self.output_text = TruthKeyValue.objects.filter(
                truth=truth).filter(key=key)[0].value
            self.output()
        except Exception, e:
