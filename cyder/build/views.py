...
from cyder.middleware.restrict_to_remote import allow_build
import models
...
def build_attribute_form_generator(show_fields):
    class f(forms.ModelForm):
        purposes = forms.ModelMultipleChoiceField(
            queryset=models.BuildPurpose.objects.all(),
...
def build_show(request, id):
    system = get_object_or_404(
        models.System, pk=id, allocation__name='release')
...
def build_bulk(request):
    systems = models.System.build_objects.filter(
        id__in=request.GET.getlist('system_id'))
    if request.method == 'POST':
        ...
        if build_attribute_form_generator(save_fields)(request.POST).is_valid():
            for s in systems:
                try:
                    ...
                    build_attribute = None
                form = build_attribute_form_generator(
                    save_fields)(request.POST, instance=build_attribute)
                ba = form.save(commit=False)
                ba.system = s
                ba.save()
                form.save_m2m()
...
def build_list(request):
    system_list = models.System.build_objects.select_related(
    ).order_by('hostname')
