...
from cyder.dnsutils.svn_build import get_forward_svn_sites_changed
...
def do_dns_build():
    ...
    rev_svn_zones = collect_rev_svn_zones(
        settings.REV_SITE_PATH, settingsZONE_PATH)
...
def zone_build_from_config(job=None):
    ...
    if job == "zones":
        from cyder.dnsutils.migrate.zone_configs.zones import zones
        for config in zones:
            zone_path = config['path']
            root_domain = config['zone_name']
            name_reversed = config['name_reversed']
    ...
    try:
        if job == "mozilla_org":
            from cyder.dnsutils.migrate.zone_configs.mozilla_org import mozilla_org
    ...
    if job == "dc":
        from cyder.dnsutils.migrate.zone_configs.mozilla_com_dc_zone_config import mozilla_com_dcs
    ...
    if job == "private_reverse":
        from cyder.dnsutils.migrate.zone_configs.private_reverse import private_reverse
...
def build_forward(sites_to_build, inv_forward, svn_zones):
    ...
    for vlan_site, network, site_path in sites_to_build:
        ...
        # [<dnsutils.build_nics.Interface object at 0xab54250>]),...]
