...
class OncallTest(TestCase):
    ...
    def test_desktop_oncall_count(self):
        oncalls = User.objects.select_related(
    ...
    def test_oncall_index_page(self):
        ...
        self.assertEqual(
            resp.context['current_desktop_oncall'], 'user1@domain.com')
        self.assertEqual(
            resp.context['current_sysadmin_oncall'], 'user2@domain.com')
    ...
    def test_updating_sysadmin_oncall(self):
        ...
        resp = self.client.get('/systems/oncall/', follow=True)
        self.assertEqual(
            resp.context['current_desktop_oncall'], 'user2@domain.com')
        self.assertEqual(
            resp.context['current_sysadmin_oncall'], 'user1@domain.com')
