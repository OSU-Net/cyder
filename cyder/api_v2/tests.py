...
class TestOnCall(TestCase):
    ...
    def test_get_current_desktop_oncall_email(self):
        ...
        resp = self.client.put(
            '/en-US/api/v2/oncall/setsysadmin/user1@domain.com/', follow=True)
        self.assertEqual(200, resp.status_code)
        resp = self.client.get('/api/v2/oncall/desktop/email/', follow=True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_get_current_desktop_oncall_irc_nick(self):
        resp = self.client.get(
            '/api/v2/oncall/sysadmin/irc_nick/', follow=True)
        self.assertEqual(200, resp.status_code)
        obj = json.loads(resp.content)
    ...
    def test_set_services_oncall(self):
        ...
        resp = self.client.put(
            '/en-US/api/v2/oncall/setservices/user4@domain.com/', follow=True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_set_desktop_oncall(self):
        ...
        resp = self.client.put(
            '/en-US/api/v2/oncall/setdesktop/user3@domain.com/', follow=True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_set_desktop_oncall_by_irc_nick(self):
        ...
        resp = self.client.put(
            '/en-US/api/v2/oncall/setdesktop/user3/', follow=True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_set_sysadmin_oncall(self):
        ...
        resp = self.client.put(
            '/en-US/api/v2/oncall/setsysadmin/user3@domain.com/', follow=True)
        self.assertEqual(200, resp.status_code)
    ...
    def test_set_sysadmin_oncall_by_irc_nick(self):
        ...
        resp = self.client.put(
            '/en-US/api/v2/oncall/setsysadmin/user3/', follow=True)
        self.assertEqual(200, resp.status_code)
...
class SystemApi(TestCase):
    ...
    def test_get_system_by_hostname(self):
        resp = self.client.get(
            '/api/v2/system/asfdasfasfasdfasfasdfsadf/', follow=True)
        self.assertEqual(404, resp.status_code)
    ...
    def test_create_system(self):
        resp = self.client.post(
            '/en-US/api/v2/system/%s/' % self.new_hostname, follow=True)
        self.assertEqual(201, resp.status_code)
        obj = json.loads(resp.content.split(" = ")[1])
        resp = self.client.get('/api/v2/system/%i/' % obj['id'], follow=True)
        self.assertEqual(200, resp.status_code)
        resp = self.client.get(
            '/api/v2/system/%s/' % self.new_hostname, follow=True)
        self.assertEqual(200, resp.status_code)
        self.assertEqual(
            json.loads(resp.content)['hostname'], self.new_hostname)
    ...
    def test_update_system(self):
        ...
        obj = json.loads(resp.content.split(" = ")[1])
        self.assertEqual(201, resp.status_code)
        resp = self.client.put('/en-US/api/v2/system/%i/' % (
            obj['id']), {'hostname': 'updated_hostname'}, follow=True)
        self.assertEqual(200, resp.status_code)
        resp = self.client.get('/api/v2/system/%i/' % (obj['id']), follow=True)
        self.assertEqual(200, resp.status_code)
        self.assertEqual(
            json.loads(resp.content)['hostname'], 'updated_hostname')
    ...
    def test_delete_system(self):
        ...
        resp = self.client.delete(
            '/en-US/api/v2/system/%i/' % (obj['id']), follow=True)
        self.assertEqual(200, resp.status_code)
        obj = json.loads(resp.content.split(" = ")[1])
        resp = self.client.get('/api/v2/system/%i/' % (obj['id']), follow=True)
        self.assertEqual(404, resp.status_code)
    ...
    def test_key_value_api(self):
        resp = self.client.get(
            '/api/v2/keyvalue/?keystore=fake-hostname2', follow=True)
        self.assertEqual(json.loads(
            resp.content)['truth:test:cluster_name'], 'Test Cluster Name')
        ...
        resp = self.client.get(
            '/api/v2/keyvalue/?key=cluster_owner', follow=True)
        self.assertEqual(json.loads(
            resp.content)['truth:test:cluster_owner'], 'The Cluster Owner')

        resp = self.client.get(
            '/api/v2/keyvalue/?value=10.99.32.3', follow=True)
        self.assertEqual(json.loads(resp.content)['host:fake-hostname1:nic.0.ipv4_address.0'], '10.99.32.3')

    def test_search_by_asset_tag(self):
        resp = self.client.get('/api/v2/system/3/', {'search':
                               True, 'asset_tag': '65432'}, follow=True)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(json.loads(resp.content)[0]['asset_tag'], '65432')
        self.assertEqual(
            json.loads(resp.content)[0]['hostname'], 'fake-hostname2')

    def test_search_by_serial(self):
        resp = self.client.get('/api/v2/system/3/', {'search':
                               True, 'serial': '39993'}, follow=True)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(json.loads(resp.content)[0]['serial'], '39993')
        self.assertEqual(
            json.loads(resp.content)[0]['hostname'], 'fake-hostname1')
    ...
    def test_search_by_system_rack(self):
        resp = self.client.get('/api/v2/system/3/', {'search':
                               True, 'system_rack_id': '1'}, follow=True)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(
            json.loads(resp.content)[0]['hostname'], 'fake-hostname1')
    ...
    def test_search_by_system_switch_ports(self):
        resp = self.client.get('/api/v2/system/3/', {'search':
                               True, 'switch_ports': '101.02'}, follow=True)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(
            json.loads(resp.content)[0]['hostname'], 'fake-hostname1')
    ...
    def test_delete_key_by_system(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(
            int(json.loads(resp.content.split(" = ")[1])['id']), 14)
...
class DHCPApi(TestCase):
    ...
    def test_get_single_scope(self):
        resp = self.client.get(
            '/api/v2/keyvalue/?key_type=dhcp_scopes', follow=True)
    ...
    def test_get_system_by_scope(self):
        ...
        self.assertEqual(
            system_list[0]['nic.0.mac_address.0'], '00:00:00:00:00:AA')
        self.assertEqual(system_list[0]['nic.0.ipv4_address.0'], '10.99.32.1')
        self.assertEqual(
            system_list[0]['nic.1.mac_address.0'], '00:00:00:00:00:BB')
    ...
    def test_add_dhcp_scope_via_api(self):
        ...
        resp = self.client.post(
            '/en-US/api/v2/dhcp/phx1-vlan73/add_scheduled_task/', follow=True)
        self.assertEqual(resp.status_code, 200)
...
class TestReverseDNSApi(TestCase):
    ...
    def test_get_single_reverse_zone_names_with_descriptions(self):
        ...
        self.assertEqual(
            scope_list[0]['description'], '10.99.32.0 reverse dns zone')
        self.assertEqual(scope_list[1]['name'], 'phx1-33.8.10.in-addr.arpa')
        self.assertEqual(
            scope_list[1]['description'], '10.99.33.0 reverse dns zone')
