...
class UserSystemForm(forms.ModelForm):
    ...
    date_purchased = forms.DateField(widget=SelectDateWidget(
        years=range(1999, datetime.today().year + 2)), initial=datetime.now())
    loaner_return_date = forms.DateField(
        widget=SelectDateWidget(), initial=datetime.now(), required=False)
...
class UserLicenseForm(forms.ModelForm):
    ...
    class Meta:
        model = UserLicense
        fields = ['username', 'version', 'license_type',
                  'license_key', 'owner', 'user_operating_system']
